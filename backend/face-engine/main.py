...
async def enroll(
    ...
    for file in files:
        ...
        face = extractFace(img)
        if face is None:
            return JSONResponse({"success": False, "error": "No face detected in: " + file.filename})
...
async def recognize(file: UploadFile):
    ...
    probe_emb = embed_arcface(face).reshape(-1)

    best_id = None
    best_name = None
    best_score = -1

    for staffId, info in DB.items():
        stored_emb = np.array(info["embedding"], dtype=np.float32).reshape(-1)
        sim = float(np.dot(stored_emb, probe_emb))
